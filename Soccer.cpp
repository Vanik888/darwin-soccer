...
bool Soccer::getBallCenter(double &x, double &y) {
  ...
  if ( ! find ) {
    ...
    std::cout << "(" << x << "," << y << ");" << "-real x and y;" << endl;
    x = 2.0 * x / width  - 1.0;
    y = 2.0 * y / height - 1.0;
    std::cout << "(" << x << "," << y << ");" << "2*x/width -1.0;" << endl;
    return true;
  }
}
...
void Soccer::run() {
  ...
  greeting(); // Make a greeting movements
  ...
  while (true) {
    ...
    // go in the direction of the ball and kick it
      ...
      x  = 0.015*x + px;
      y  = 0.015*y + py;
      std::cout << "(" << x << "," << y << "); "<< "0.015*y + py;" << "(" << px << "," << py << "); " << endl;
      px = x;
      py = y;
      ...
      if (y < 0.1) { // ball far away, go quickly
        ...
        std::cout << "(" << x << "," << y << "); " << "(" << px << "," << py << "); ball is so far" << endl;
      }
      else {// ball close, go slowly
        mGaitManager->setXAmplitude(0.5);
        std::cout << "(" << x << "," << y << "); " << "(" << px << "," << py << "); ball is so close " << endl;
      }
      ...
      if (y > 0.35) {
        ...
        // set eye led to green
        mEyeLED->set(0x00FF00);
//        mGaitManager->setMoveAimOn(true);
        mGaitManager->start();
